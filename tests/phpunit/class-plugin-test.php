...
class Plugin_Test extends \WP_UnitTestCase {
	...
	public function test_post_type_has_public_agent_skill_permalinks_and_is_available_in_rest() {
		$post_type = get_post_type_object( Plugin::POST_TYPE );
		...
		$this->assertSame( Plugin::PERMALINK_PREFIX, $post_type->rewrite['slug'], 'Agent Skill permalinks should use the requested singular prefix.' );
		$this->assertFalse( $post_type->rewrite['with_front'], 'Agent Skill permalinks should not inherit the site front base.' );
		$this->assertSame( Plugin::PERMALINK_PREFIX, $post_type->query_var, 'Agent Skill plain permalinks should use the requested query variable.' );
		$this->assertTrue( post_type_supports( Plugin::POST_TYPE, 'revisions' ), 'Skill revisions should be enabled.' );
		$this->assertTrue( post_type_supports( Plugin::POST_TYPE, 'custom-fields' ), 'Agent Skill post meta should be available to the REST API for editor saves.' );
	...
	public function test_plugin_action_links_point_at_the_skill_list_and_settings() {
		...
		$this->assertSame( admin_url( 'edit.php?post_type=' . Plugin::POST_TYPE ), $this->plugin->get_skills_admin_url(), 'The skills admin URL should open the Agent Skill post list.' );
	}

	public function test_registers_compatibility_post_meta() {
		$registered_meta = get_registered_meta_keys( 'post', Plugin::POST_TYPE );
	...
	public function test_rest_response_includes_the_skill_links() {
		...
		$discovery = new Discovery( new Skills( Plugin::POST_TYPE ), new Response_Emitter( new Request() ) );
	...
	public function test_skill_editor_keeps_the_full_block_catalog_available() {
		$skill_context = new \WP_Block_Editor_Context( [ 'post' => get_post( self::factory()->post->create( [ 'post_type' => Plugin::POST_TYPE ] ) ) ] );
